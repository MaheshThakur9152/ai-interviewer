...
const SignIn: React.FC<SignInProps> = ({ onBack, onComplete }) => {
  ...
  const [step, setStep] = useState<'input' | 'biometric'>('input');
            ...
            <div className="space-y-8 md:space-y-10">
              ...
              {step === 'input' ? (
                <div className="space-y-6">
                  <form onSubmit={handleEstablishLink} className="space-y-4">
                    ...
                    <button className="w-full bg-white text-black font-bold py-5 rounded-2xl hover:bg-gray-200 transition-all shadow-[0_0_30px_rgba(255,255,255,0.2)] flex items-center justify-center gap-3 group/btn">
                      <span className="text-[11px] tracking-[0.3em] uppercase">Initiate Neural Sync</span>
                ...
                <div className="space-y-12 py-4">
                  <div className="flex flex-col items-center">
                    <div className="relative group cursor-pointer" onClick={handleCompleteAuth}>
                      <div className={`w-40 h-40 rounded-full border-2 border-dashed border-purple-500/20 flex items-center justify-center transition-all duration-[3s] ${isSyncing ? 'animate-spin' : 'hover:scale-105'}`}>
                        <div className="w-28 h-28 rounded-full bg-purple-500/5 flex items-center justify-center relative overflow-hidden">
                           ...
                           <Fingerprint size={50} className={`${isSyncing ? 'text-purple-300' : 'text-purple-500'} transition-colors animate-pulse`} />
                        </div>
                      </div>
                      {isSyncing && (
                    ...
                    <div className="mt-8 text-center">
                       <p className="text-[9px] font-bold text-purple-400 uppercase tracking-[0.5em] animate-pulse">Bio-Identity Check Required</p>
                    </div>
                  </div>
                  ...
                  {!isSyncing && (
                    <button 
                      ...
                      className="w-full py-4 text-[9px] font-bold uppercase tracking-[0.4em] text-gray-500 hover:text-white transition-colors"
                    >
                      Reset Auth Token
                    </button>
                  )}
